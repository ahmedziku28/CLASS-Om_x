...
# --- CHANGE 1: FORCE GSL LINKING ---
# GSL flags (Try auto-detect, but force manual path if needed)
GSL_CFLAGS = $(shell gsl-config --cflags)
GSL_LIBS   = $(shell gsl-config --libs)

# Manual override since we know where it is:
# We add -L/usr/lib64 explicitly.
LIBS = -lm -L/usr/lib64 -lgsl -lgslcblas

# Add GSL_LIBS to LIBS just in case config found something extra
LIBS += $(GSL_LIBS)

# all other compilation flags
CCFLAG = -g -fPIC -std=c99 -D_GNU_SOURCE $(GSL_CFLAGS)
LDFLAG = -g -fPIC
...
class: $(TOOLS) $(SOURCE) $(EXTERNAL) $(OUTPUT) $(CLASS)
	$(CPP) $(OPTFLAG) $(OMPFLAG) $(LDFLAG) -o class $(addprefix build/,$(notdir $^)) $(LIBS)
...
classy: libclass.a python/classy.pyx python/cclassy.pxd
	# --- CHANGE 3: EXPORT FLAGS TO PYTHON ---
	export CC=$(CC); \
	export LDFLAGS="-L/usr/lib64 -lgsl -lgslcblas"; \
